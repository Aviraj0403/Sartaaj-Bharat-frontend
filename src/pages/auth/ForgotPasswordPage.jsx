...
const ForgotPasswordPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.message || "Recovery initialization failed.",
      );
    } finally {
        ...
        {!isSent ? (
          ...
          >
            ...
            <div className="space-y-2">
              <h3 className="text-white text-lg font-black italic uppercase tracking-tight">
                Recovery Sent
              </h3>
              <p className="text-slate-400 text-xs font-medium tracking-wide leading-relaxed">
                An encrypted recovery link has been dispatched to{" "}
                <span className="text-blue-400 font-bold">{email}</span>. Please
                verify your inbox.
              </p>
            </div>
            <button
