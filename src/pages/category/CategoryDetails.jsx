    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-[11px] font-black uppercase tracking-widest text-slate-900 flex items-center gap-2">
          <SlidersHorizontal size={16} className="text-blue-600" /> Refined
          Search
        </h3>
        {hasFilters && (
          <button
          ...
          <div className="flex flex-col gap-2">
            ...
            {filterOpts.subCategories.map((sub) => (
              <button
                key={sub.id}
                onClick={() =>
                  setSelectedSubCat(selectedSubCat === sub.id ? "" : sub.id)
                }
      ...
      {/* Dynamic Filters */}
      {filterOpts?.dynamicFilters &&
        Object.entries(filterOpts.dynamicFilters).map(
                  ...
                  >
                    <div className="relative flex items-center justify-center">
                      <input
                        ...
                        checked={
                          dynamicFilters[`filter_${filterName}`]
                            ?.split(",")
                            .includes(val) || false
                        }
        ...
        <div className="container-custom relative py-10 md:py-14">
          ...
          >
            ...
            <p className="text-slate-400 text-sm max-w-xl leading-relaxed mb-8 italic opacity-70">
              Engineering the future of {categoryName?.toLowerCase()}. Curated
              for the tech sovereign.
            </p>

            <div className="flex flex-wrap gap-8 items-center">
              <div className="text-center">
                <span className="text-blue-500 text-3xl font-black italic block">
                  {products.length}
                </span>
              ...
              {filterOpts?.subCategories?.length > 0 && (
                <>
                  ...
                  <div className="text-center">
                    <span className="text-white text-base font-black italic block">
                      {filterOpts.subCategories.length}
                    </span>
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em]">
                      Sub-categories
                    </span>
            ...
            <div className="bg-white p-3.5 rounded-2xl border border-slate-100 shadow-sm flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  {hasFilters && (
                    <span className="bg-blue-600 text-white rounded-full text-[8px] w-4 h-4 flex items-center justify-center font-black">
                      !
                    </span>
                  )}
                </button>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {Object.entries(dynamicFilters).map(([key, valueStr]) => {
                  ...
                  return values.map((val) => (
                    <span
                      key={`${key}-${val}`}
                      className="bg-emerald-50 text-emerald-600 border border-emerald-100 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5"
                    >
                      {filterName}: {val}{" "}
                      <X
                        size={10}
                        className="cursor-pointer"
                        onClick={() => toggleDynamicFilter(filterName, val)}
                      />
                    </span>
                  ));
                })}
            ...
            <AnimatePresence mode="wait">
              {isLoading ? (
                ...
                >
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-5 italic">
                    Loading products…
                  </span>
                </motion.div>
              ) : products.length > 0 ? (
                ...
                >
                  ...
                  <button
                    onClick={clearFilters}
                    className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-700 transition-all shadow-xl shadow-blue-200 italic"
                  >
                    Clear Filters
                  </button>
                </motion.div>
        ...
        {isMobileFilterOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 bottom-0 w-[85%] max-w-sm bg-slate-50 z-[60] flex flex-col shadow-2xl"
            >
