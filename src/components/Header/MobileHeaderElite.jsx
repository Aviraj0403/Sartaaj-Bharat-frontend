...
const MobileHeaderElite = () => {
  ...
  return (
    <div className="lg:hidden">
      {/* Top Sticky Bar */}
      <header
        className={`fixed top-0 left-0 w-full z-70 transition-all duration-300 ${isScrolled ? "bg-white/90 backdrop-blur-xl shadow-md py-3" : "bg-white py-4"}`}
      >
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="flex flex-wrap gap-2 mb-12">
                {["Headphones", "MacBook", "iPhone", "Watch", "Gaming"].map(
                  (tag) => (
                    <button
                      key={tag}
              ...
              <div className="flex flex-col gap-6">
                {categories?.slice(0, 4).map((cat) => (
                  <Link
                    ...
                    to={`/category/${cat.slug}`}
                    onClick={() => setIsSearchOpen(false)}
                    className="text-xl font-black italic hover:text-blue-600 transition-colors flex items-center justify-between"
                  >
                    {cat.name}
